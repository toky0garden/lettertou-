...
import { postUpdateProfile, postUploadAvatar } from '@/utils/api/request';
...
export default function EditProfilePage() {
  ...
  if (!config.user || config.user.username.toLowerCase() !== username.toLowerCase()) {
    return (
      <main className='container-wrapper min-h-screen px-4 pt-28 text-center'>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (avatar) user = (await postUploadAvatar('avatar', avatar)).data;
      if (banner) user = (await postUploadAvatar('banner', banner)).data;
        ...
        <Card className='mx-auto w-full max-w-2xl'>
          ...
          <CardContent className='flex flex-col gap-7'>
            <section className='flex flex-col gap-4'>
              ...
              <div className='bg-muted relative h-32 overflow-hidden rounded-xl border'>
                ...
                <Avatar className='ring-background absolute bottom-3 left-4 size-20 ring-4'>
            ...
            <section className='flex flex-col gap-4 border-t pt-6'>
              <div>
                ...
                <p className='text-muted-foreground text-sm'>
                  Оставьте новые поля пустыми, если пароль менять не нужно.
                </p>
              </div>
              <div className='flex flex-col gap-2'>
