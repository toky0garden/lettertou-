...
import type { SwiperBannerSchema } from '@/shared/api/types.gen';
import { ROUTES, typeMapping } from '@/app/(constants)';
import { Badge, Button, Typography } from '@/components/ui';
import { cn } from '@/utils/lib/utils';

interface HomeHeroProps {
  banners: SwiperBannerSchema[];
}
      ...
      <div className='relative overflow-hidden rounded-3xl border shadow-sm'>
        <div className='bg-muted absolute inset-0'>
          <Image
            key={banner.id}
            alt=''
            className='size-full object-cover object-center blur-sm'
            fill
            priority
            quality={85}
            sizes='100vw'
            ...
            <div className='flex flex-wrap items-center gap-2'>
              ...
              {banner.type && (
                <Badge variant='secondary' className='backdrop-blur-sm'>
                  {typeMapping[banner.type] ?? banner.type}
                </Badge>
              )}
            </div>
            ...
            <div className='flex items-center justify-between gap-4'>
              ...
              <div className='flex gap-2'>
                <button
                  ...
                  className='flex size-9 items-center justify-center rounded-md bg-white/15 text-white backdrop-blur-sm transition-colors hover:bg-white/30'
                >
                ...
                <button
                  ...
                  onClick={() => setActive((current) => (current + 1) % banners.length)}
                  className='flex size-9 items-center justify-center rounded-md bg-white/15 text-white backdrop-blur-sm transition-colors hover:bg-white/30'
                >
